...
interface VendingState {
    // return type is VendingState for all
    ...
    String getStateName(); // only prints the state name in console
}
...
class VendingMachine {
    ...
    // only for seeing the output in the console
...
class NoCoinState implements VendingState {
    ...
    public VendingState refill(VendingMachine machine, int quantity) {
        ...
        machine.incrementItemCount(quantity); // increments itemCount by the number of quantity given
        return machine.getNoCoinState(); // Stay in same state as machine is refilled but state is not changed
    }
...
class HasCoinState implements VendingState {
    public VendingState insertCoin(VendingMachine machine, int coin) {
        ...
        return machine.getHasCoinState(); // Stay in same state as more coins can be taken
    }
    
    public VendingState selectItem(VendingMachine machine) {
        // we need to add a validation check to make sure that inserted money is equal to or more than cost of item
        ...
        if (machine.getInsertedCoin() >= machine.getPrice()) {
            ...
            // money inserted is LESS than cost of item
...
class SoldOutState implements VendingState {
    // This state can only transition to refillState
